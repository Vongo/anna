...
The user can modify the chosen category at anytime since the drop-down list is available on the discussion page of the UI.
\begin{figure}[!h]
\begin{center}
...
On of the NLP application we wanted to try with this tool was to build generic sentences.\\
Here, we tried to detect proper nouns and to replace them with some chosen values. The first problem we faced concern the NLP library : \textbf{LEO}
Then, we figured out that replacing proper nouns isn't that simple : it depends of the structure of the sentence. For instance, in the sentence \say{Hello Jack, I'm Bob ! What do you think about our friend Lola ?} we got 3 NNP but we can't replace them with the same value, it would make no sense. We had to split the initial utterance into sub-phrases. This was achieved by detecting the presence of a grammatical separator \textbf{INSERER LISTE ADRIAN}.\\
Once we isolated the sub-phrases, we determined if the NNP was the subject of the sentence (\say{I'm Bob !}), if someone is talking to him (\say{Hello Jack,}) or if he's a third party (\say{What do you think about our friend Lola ?}). Given these three cases, we replaced as follows :
\begin{description}
    \item[Case 1 : Subject] NNP becomes \say{Anna}.
    \item[Case 2 : Interlocutor] NNP becomes \say{USERNAME}
    \item[Case 3 : Third party] NNP becomes \say{Behrang}
\end{description}
\textbf{INSERER FIGURE}
