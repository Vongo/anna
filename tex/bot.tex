...
For instance, in the sentence \say{Hello Jack, I'm Bob ! What do you think about our friend Lola ?} we got 3 NNP (Jack, Bob and Lola) but we can't replace them with the same value, it would make no sense.
We had to split the initial utterance into sub-sentences, or naive propositions.
This was done by detecting the presence of a grammatical separators such as "\textbf{.}", "\textbf{that}", "\textbf{:}", "\textbf{;}", "\textbf{?}", "\textbf{!}", "\textbf{,}", "\textbf{what}", "\textbf{-}" or "\textbf{...}".\\
Once we isolated the sub-sentences, we determined whether the NNP was the subject of the sentence (\say{I'm Bob !}), if someone is talking to him (\say{Hello Jack,}) or if he's a third party (\say{What do you think about our friend Lola ?}).
Given these three cases, we replaced as follows :
\begin{description}
